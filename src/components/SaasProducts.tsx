...
import { Store, School, Calendar } from "lucide-react";
    ...
    <section id="produtos" className="py-24 bg-agency-dark-lighter relative">
      ...
      <div className="container mx-auto relative z-10">
        ...
        <Tabs defaultValue="ecommerce" className="max-w-5xl mx-auto">
          <TabsList className="grid grid-cols-3 mb-12 bg-agency-dark">
            {saasProducts.map((product) => {
              const IconComponent = iconMap[product.id as keyof typeof iconMap];
              return (
                <TabsTrigger
                  key={product.id}
                  value={product.id}
                  className="data-[state=active]:bg-agency-red data-[state=active]:text-white"
                >
                  <IconComponent className="mr-2 h-5 w-5" />
                  <span>
                    {product.id === "ecommerce"
                      ? "Loja Virtual"
                      : product.id === "reservation"
                      ? "Reserva de Assentos"
                      : "Gestão Escolar"}
                  </span>
                </TabsTrigger>
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                  ...
                  <Button className="bg-agency-red hover:bg-agency-red-dark text-white">
                    Saiba Mais
                  </Button>
                </div>
